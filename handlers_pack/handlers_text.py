...
async def text_youtube_link_handler(message: Message):
    ...
    for url in youtube_urls:
        ...
        if not video_data:
            ...
            await add_video(yt, audio_file_id, photo_file_id, text, thesis)

            markup = get_person_video_card_keyboard(yt.video_id, EMPTY_LIKE_ICON, 0)
        ...
        if video_data:
            ...
            like_icon = get_like_icon_for_post(yt.video_id, await get_user_likes(message.from_user.id))
            markup = get_person_video_card_keyboard(yt.video_id, like_icon, video_data['likes'])
            ...
            if preview_file_id:
                ...
                await set_video_data(yt.video_id, video_data)
